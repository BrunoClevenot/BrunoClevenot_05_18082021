...
fetch("http://localhost:3000/api/cameras/" + idTrouvee)
  ...
  .then(function (value) {
    if ("content" in document.createElement("template")) {
      ...
      clone
        ...
        .addEventListener("change", (ev) => globalPrice(ev, value));
  ...
  .catch(function (err) {
    if (err == "TypeError: value is undefined") {
      ...
      let clone = document.importNode(template.content, true);
      clone.querySelector(".slot-noData-1").textContent =
        "Désolé, aucun produit n'a été trouvé dans la base.";
      clone.querySelector(".slot-noData-2").textContent =
...
function addCamera(camera) {
  ...
  for (let i = 0; i < cameras.length; i++) {
    ...
    if (
      cameras[i].id === idTrouvee &&
      cameras[i].lentille === camera.lentille
    ) {
      console.log(cameras[i].id);
      console.log(idTrouvee);
...
document.querySelector("#validePanier").addEventListener("click", (ajout) => {
  ...
  // On crée un objet
  const camera = new AppareilPhoto(
    idTrouvee,
    image,
    nom,
    lentille,
